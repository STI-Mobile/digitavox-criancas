...
O engine de jornada é testado com `docs/examples/journey_course.json`: ordem dos arrays (inclusive reordenação), retomada após recriar estado, avanço entre lições/módulos, atividades indisponíveis, personagens herdados/substituídos, áudio por etapa, navegação rápida e falhas de mídia. Testes de widget cobrem navegação hierárquica, retorno do sistema, nomes/ações semânticas dos botões, continuação por teclado, texto em escala 2 e fallback de imagem ausente. O exemplo usa caminhos de mídia ilustrativos; não comprova a presença desses assets no bundle nem a reprodução física.
