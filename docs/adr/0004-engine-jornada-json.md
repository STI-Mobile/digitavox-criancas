...
As cenas opcionais (`scene`) contêm texto, configuração declarativa `audioGuidance` e referência a personagem. Personagens são definidos em `course.characters`, com imagem local e descrição acessível. Referências são validadas antes de abrir o curso. A imagem herda a referência do ancestral mais próximo; áudio e texto pertencem apenas à etapa atual.

Toda transição descarta a sessão antiga antes de enviar o evento da nova etapa ao `CourseAudioOrchestrator`. O widget de exercício não possui nem descarta a sessão. Isso evita que o descarte atrasado de um widget interrompa o áudio da próxima etapa. A sessão apenas informa o resultado da entrada; o engine escolhe o evento declarativo correspondente. Navegação rápida cancela sequências obsoletas pelo orquestrador. Saída para segundo plano interrompe o áudio, sem reinício automático.
...
- Não há interpretação de scripts, roteador externo nem importação remota.
